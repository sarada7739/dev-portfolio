...
2026-09-20 | UI | 制作物カードは href 未指定（#）ならクリックでライトボックス、href ありならリンク。ネイティブ dialog を使用 | T-022
2026-09-20 | 品質 | T-022 レビューは開いた状態と操作だけ実測し、閉じた状態のグリッドを見ていなかった。UI 変更のレビューは変更前後のスクリーンショット比較を必須にする | T-023
